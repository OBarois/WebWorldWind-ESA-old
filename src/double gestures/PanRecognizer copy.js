...
define(['../gesture/GestureRecognizer'],
    function (GestureRecognizer) {
        ...
        PanRecognizer.prototype.touchStart = function (event) {
            ...
            // if ( this.numberOfTaps > 1 && this.tapCounter > 1) this.failAfterDelay(this.maxTapInterval)
            this.failAfterDelay(this.maxTapInterval)
        ...
        PanRecognizer.prototype.touchEnd = function (touch) {
            if (this.touchCount == 0 &&  (this.numberOfTaps - this.tapCounter)  == 0) { // last touch ended and not waiting for another one
                if (this.state == WorldWind.POSSIBLE) {
        ...
        PanRecognizer.prototype.touchCancel = function (touch) {
            if (this.touchCount == 0 && (this.numberOfTaps - this.tapCounter)  == 0) { // last touch cancelled and not waiting for another one
                if (this.state == WorldWind.POSSIBLE) {
                    this.state = WorldWind.FAILED;
                } else if (this.state == WorldWind.BEGAN || this.state == WorldWind.CHANGED) {
                    console.log("pan  cancel (numberOfTaps: "+this.numberOfTaps+" / tapCounter: "+this.tapCounter+" / touches: "+this.touchCount+" min/max: "+this.minNumberOfTouches+"/"+this.maxNumberOfTouches+")")
